...
class Synthesizer(nn.Module):
    ...
    """
    def __init__(self, args, nclass, nchannel, hs, ws, rank):
        super().__init__()
        self.ipc = args.ipc
        self.nclass = nclass
        ...
        self.data = nn.Parameter(torch.randn(size=(self.nclass * self.ipc, self.nchannel, hs, ws),
                                dtype=torch.float),
                                requires_grad=True)
...
def diffaug(args, rank):
    """Differentiable augmentation for condensation
    """
    aug_type = args.aug_type
    normalize = utils.Normalize(mean=MEANS[args.dataset], std=STDS[args.dataset], device=rank)
...
def condense(args, logger):
    ...
    for i in range(args.nclass):
        img, label = loader_real.class_sample(i, length_list[i])
    ...
    if args.start_interval > 0:
        previous_images, previous_labels = torch.load(os.path.join(args.save_dir, f'interval_{args.start_interval - 1}_data.pt'), map_location=f'cuda:{device}')
    else:
    ...
    for interval_idx in range(args.start_interval, args.num_intervals):
        ...
        if previous_images is not None:
            ...
            with torch.no_grad():
                ...
                synset.data.data = torch.tensor(new_data,
                                dtype=torch.float,
                                requires_grad=True,
        ...
        if interval_idx >= 1:
            for i in range(interval_idx):
                prev_data, prev_targets = torch.load(os.path.join(args.save_dir, f'interval_{i}_data.pt'), map_location=f'cuda:{device}')
        ...
        # Data distillation
        synset = DDP(synset, device_ids=[device])
        ...
        for it in range(n_iter):
            ...
            synset.module.data.data = torch.clamp(synset.module.data.data, min=0., max=1.)
            ...
            if (it + 1) in it_test:
                previous_images = synset.module.data.data.clone()
                previous_labels = synset.module.targets.data.clone()
                if rank == 0:
                ...
                torch.save(
                    [synset.module.data.detach().cpu(), synset.module.targets.cpu()],
